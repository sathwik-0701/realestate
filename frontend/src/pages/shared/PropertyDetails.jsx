...
import { HiBackspace, HiBadgeCheck, HiCalendar, HiChatAlt, HiChevronLeft, HiChevronRight, HiCollection, HiHeart, HiLocationMarker, HiOutlineHeart, HiOutlineHome, HiOutlineUserGroup, HiOutlineViewGrid, HiX } from 'react-icons/hi';
...
const PropertyDetails = () => {
  ...
  const [lightboxIndex, setLightboxIndex] = useState(null); // ✅ Fix 1: moved to top, was declared after early returns
  ...
  const handelWishlistToggle = async () => {
    ...
    try { // ✅ Fix 2: try block was opened inside if(!user), catch was outside — braces were mismatched
      if (isInWishlist) {
    ...
    } catch (err) { // ✅ Fix 3: catch was floating outside try — now properly paired
      alert("Failed to update wishlist.");
    }
  }; // ✅ Fix 4: closing brace for handelWishlistToggle was missing
  ...
  const handleChatStart = async () => {
    if (!user) return navigate("/login");
    if (user.role !== "buyer") { // ✅ Fix 5: return alert() was before try block — code after return never ran
      return alert("Only buyers can chat with sellers");
    }
    try { // ✅ Fix 6: try was nested inside the role-check block, now at correct level
      ...
      await axios.post(
        `${API_URL}/api/chat/send`,
        {
          ...
          image: property.images[0], // ✅ Fix 8: "Image" → "image" (consistent casing)
        },
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    <div className={s.pageContainer}>
      ...
      <main className={s.mainContainer}>
        <nav className={s.breadcrumbs}>
          ...
          <HiChevronRight />
          <Link to="/properties" className={s.breadcrumbLink}>Listing</Link>
          <HiChevronRight />
        ...
        <div className={s.galleryContainer}>
          <div
            ...
            style={{ // ✅ Fix 9: style prop was floating outside any JSX element
              gridTemplateColumns:
          ...
          >
            ...
            >
              <img
                src={property.images[0]} // ✅ Fix 10: missing closing } on src
                alt="property image"
        ...
        {/* Lightbox model*/}
        {lightboxIndex !== null && (
            <div className={s.lengthboxOverlay} onClick={closeLightbox}>
                ...
                <div className={s.infoHeader}>
                    ...
                    <div className={s.actionButtons}>
                        {(!user || user.role == "buyer") && (
                            <button onClick={handelWishlistToggle}
